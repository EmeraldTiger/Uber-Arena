...
static void CG_Obituary( entityState_t *ent ) {
	...
	if ( attacker != ENTITYNUM_WORLD ) {
		...
		case MOD_MULTI_GRENADE:
			message = "couldn't stay away from";
			message2 = "'s multi grenades";
			break;
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_EXPSHOTGUN_PICKUP:
		DEBUGNAME("EF_EXPSHOTGUN_PICKUP");
		cg.snap->uberSGUsers[cent->currentState.clientNum] = qtrue;
		break;

	case EV_EXPSHOTGUN_DROP:
		DEBUGNAME("EF_EXPSHOTGUN_DROP");
		cg.snap->uberSGUsers[cent->currentState.clientNum] = qfalse;
		break;
	
	case EV_MULTIGRENADE_PICKUP:
		DEBUGNAME("EF_MULTIGRENADE_PICKUP");
		cg.snap->uberGLUsers[cent->currentState.clientNum] = qtrue;
		break;

	case EV_MULTIGRENADE_DROP:
		DEBUGNAME("EF_MULTIGRENADE_DROP");
		cg.snap->uberGLUsers[cent->currentState.clientNum] = qfalse;
		break;

	case EV_HOMINGROCKET_PICKUP:
		DEBUGNAME("EF_HOMINGROCKET_PICKUP");
		cg.snap->uberRLUsers[cent->currentState.clientNum] = qtrue;
		break;

	case EV_HOMINGROCKET_DROP:
		DEBUGNAME("EF_HOMINGROCKET_DROP");
		cg.snap->uberRLUsers[cent->currentState.clientNum] = qfalse;
		break;

	case EV_ARCLIGHTNING_PICKUP:
		DEBUGNAME("EF_ARCLIGHTNING_PICKUP");
		//cent->uberState |= UBER_LIGHTNING_ORB;
		break;

	case EV_IONPLASMA_PICKUP:
		DEBUGNAME("EF_IONPLASMA_PICKUP");
		//cent->uberState |= UBER_PLASMA_ORB;
		break;

	case EV_TOXICRAIL_PICKUP:
		DEBUGNAME("EF_TOXICRAIL_PICKUP");
		//cent->uberState |= UBER_RAIL_ORB;
		break;

	case EV_BFG30K_PICKUP:
		DEBUGNAME("EF_BFG30K_PICKUP");
		//cent->uberState |= UBER_BFG_ORB;
		break;
