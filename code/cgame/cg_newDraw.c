...
// EMERALD: Weapon limits
static void CG_DrawPlayerWeaponLimits(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	char	num[16];
	int current, limit;
	centity_t	*cent;
	playerState_t	*ps;
	...
	limit = ps->stats[STAT_MAXWEAPONS];
	...
	if (shader) {
		...
		Com_sprintf(num, sizeof(num), "%i / %i", current, limit);
		current, limit = CG_Text_Width(num, scale, 0);
		CG_Text_Paint(rect->x + (rect->w - current) / 2, rect->y + rect->h, scale, color, num, 0, 0, textStyle);
		CG_Text_Paint(rect->x + (rect->w - limit) / 2, rect->y + rect->h, scale, color, num, 0, 0, textStyle);
	}
}
