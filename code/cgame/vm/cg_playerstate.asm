...
proc CG_CheckLocalSounds 96 12
line 297
;291:
;292:/*
;293:==================
...
LABELV $306
line 411
;411:		else if ((ps->persistant[PERS_PLAYEREVENTS] & PLAYEREVENT_POISONCREDIT) !=
ADDRLP4 88
CNSTI4 268
ASGNI4
ADDRLP4 92
...
EQI4 $310
line 412
;412:			(ops->persistant[PERS_PLAYEREVENTS] & PLAYEREVENT_POISONCREDIT)) {
line 414
;413:			// CG_Printf("%s received credit for the frag of %s");
;414:		}
LABELV $310
LABELV $307
LABELV $303
...
line 419
;417:
;418:	// check for flag pickup
;419:	if ( cgs.gametype >= GT_TEAM ) {
ADDRGP4 cgs+31456
INDIRI4
CNSTI4 3
LTI4 $312
line 420
;420:		if ((ps->powerups[PW_REDFLAG] != ops->powerups[PW_REDFLAG] && ps->powerups[PW_REDFLAG]) ||
ADDRLP4 64
CNSTI4 340
ASGNI4
ADDRLP4 68
...
LABELV $319
line 423
;421:			(ps->powerups[PW_BLUEFLAG] != ops->powerups[PW_BLUEFLAG] && ps->powerups[PW_BLUEFLAG]) ||
;422:			(ps->powerups[PW_NEUTRALFLAG] != ops->powerups[PW_NEUTRALFLAG] && ps->powerups[PW_NEUTRALFLAG]) )
;423:		{
line 424
;424:			trap_S_StartLocalSound( cgs.media.youHaveFlagSound, CHAN_ANNOUNCER );
...
NEI4 $325
line 433
;432:			// never play lead changes during warmup
;433:			if ( ps->persistant[PERS_RANK] != ops->persistant[PERS_RANK] ) {
ADDRLP4 64
CNSTI4 256
ASGNI4
ADDRFP4 0
...
EQI4 $328
line 434
;434:				if ( cgs.gametype < GT_TEAM) {
ADDRGP4 cgs+31456
INDIRI4
CNSTI4 3
GEI4 $330
line 435
;435:					if (  ps->persistant[PERS_RANK] == 0 ) {
ADDRFP4 0
INDIRP4
CNSTI4 256
ADDP4
...
NEI4 $333
line 436
;436:						CG_AddBufferedSound(cgs.media.takenLeadSound);
...
pop
line 437
;437:					} else if ( ps->persistant[PERS_RANK] == RANK_TIED_FLAG ) {
ADDRGP4 $334
JUMPV
LABELV $333
ADDRFP4 0
...
NEI4 $337
line 438
;438:						CG_AddBufferedSound(cgs.media.tiedLeadSound);
...
pop
line 439
;439:					} else if ( ( ops->persistant[PERS_RANK] & ~RANK_TIED_FLAG ) == 0 ) {
ADDRGP4 $338
JUMPV
LABELV $337
ADDRFP4 4
...
NEI4 $341
line 440
;440:						CG_AddBufferedSound(cgs.media.lostLeadSound);
...
line 448
;446:
;447:	// timelimit warnings
;448:	if ( cgs.timelimit > 0 ) {
ADDRGP4 cgs+31476
INDIRI4
CNSTI4 0
LEI4 $345
...
;449:		int		msec;
;450:
;451:		msec = cg.time - cgs.levelStartTime;
ADDRLP4 64
ADDRGP4 cg+111708
INDIRI4
ADDRGP4 cgs+34796
...
ASGNI4
line 452
;452:		if ( !( cg.timelimitWarnings & 4 ) && msec > ( cgs.timelimit * 60 + 2 ) * 1000 ) {
ADDRGP4 cg+111720
INDIRI4
CNSTI4 4
BANDI4
...
LEI4 $350
line 453
;453:			cg.timelimitWarnings |= 1 | 2 | 4;
ADDRLP4 68
ADDRGP4 cg+111720
ASGNP4
ADDRLP4 68
...
ASGNI4
line 454
;454:			trap_S_StartLocalSound( cgs.media.suddenDeathSound, CHAN_ANNOUNCER );
...
LABELV $350
line 456
;456:		else if ( !( cg.timelimitWarnings & 2 ) && msec > (cgs.timelimit - 1) * 60 * 1000 ) {
ADDRGP4 cg+111720
INDIRI4
CNSTI4 2
BANDI4
...
ASGNI4
line 458
;458:			trap_S_StartLocalSound( cgs.media.oneMinuteSound, CHAN_ANNOUNCER );
...
LABELV $357
line 460
;460:		else if ( cgs.timelimit > 5 && !( cg.timelimitWarnings & 1 ) && msec > (cgs.timelimit - 5) * 60 * 1000 ) {
ADDRGP4 cgs+31476
INDIRI4
CNSTI4 5
LEI4 $364
...
ASGNI4
line 462
;462:			trap_S_StartLocalSound( cgs.media.fiveMinuteSound, CHAN_ANNOUNCER );
...
line 467
;465:
;466:	// fraglimit warnings
;467:	if ( cgs.fraglimit > 0 && cgs.gametype < GT_CTF) {
ADDRGP4 cgs+31468
INDIRI4
CNSTI4 0
LEI4 $372
...
;482:}
LABELV $241
endproc CG_CheckLocalSounds 96 12
...
line 492
;491:	// check for changing follow mode
;492:	if ( ps->clientNum != ops->clientNum ) {
ADDRLP4 0
CNSTI4 140
ASGNI4
ADDRFP4 0
...
EQI4 $401
line 493
;493:		cg.thisFrameTeleport = qtrue;
ADDRGP4 cg+111696
CNSTI4 1
ASGNI4
line 495
;494:		// make sure we don't get any unwanted transition effects
;495:		*ops = *ps;
ADDRFP4 4
INDIRP4
ADDRFP4 0
...
line 499
;497:
;498:	// damage events (player is getting wounded)
;499:	if ( ps->damageEvent != ops->damageEvent && ps->damageCount ) {
ADDRLP4 4
ADDRFP4 0
INDIRP4
ASGNP4
...
EQI4 $404
line 500
;500:		CG_DamageFeedback( ps->damageYaw, ps->damagePitch, ps->damageCount );
ADDRLP4 12
ADDRFP4 0
INDIRP4
ASGNP4
...
;502:
;503:	// respawning
;504:	if ( ps->persistant[PERS_SPAWN_COUNT] != ops->persistant[PERS_SPAWN_COUNT] ) {
ADDRLP4 12
CNSTI4 264
ASGNI4
ADDRFP4 0
...
line 513
;512:
;513:	if ( cg.snap->ps.pm_type != PM_INTERMISSION 
ADDRGP4 cg+36
INDIRP4
CNSTI4 48
ADDP4
...
EQI4 $412
line 514
;514:		&& ps->persistant[PERS_TEAM] != TEAM_SPECTATOR ) {
line 515
;515:		CG_CheckLocalSounds( ps, ops );
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
...
line 519
;517:
;518:	// check for going low on ammo
;519:	CG_CheckAmmo();
...
;520:
;521:	// run events
;522:	CG_CheckPlayerstateEvents( ps, ops );
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
...
line 525
;523:
;524:	// smooth the ducking viewheight change
;525:	if ( ps->viewheight != ops->viewheight ) {
ADDRLP4 16
CNSTI4 164
ASGNI4
ADDRFP4 0
...
EQI4 $415
line 526
;526:		cg.duckChange = ps->viewheight - ops->viewheight;
ADDRLP4 20
CNSTI4 164
ASGNI4
ADDRGP4 cg+113044
