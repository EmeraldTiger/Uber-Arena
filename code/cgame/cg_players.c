...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	// CG_Printf("%i", cent->currentState.eFlags & EF_UW_SHOTGUN);
...
void CG_Player( centity_t *cent ) {
	...
	if (cent->currentState.eFlags & EF_UW_ROCKET) {
		cent->uberRocket = qtrue;
	}
	...
	/* if (cent->currentState.weapon == WP_GRENADE_LAUNCHER && (cg.snap->uberGLUsers[cent->currentState.clientNum] == qtrue)) {
		memcpy(&orbs, &torso, sizeof(torso));
		orbs.hModel = cgs.media.uberGrenadeOrbModel;
		orbs.customSkin = 0;
		VectorCopy(torso.origin, orbs.origin);
		spinAngles[1] = cg.time * -0.2;
		AnglesToAxis(spinAngles, orbs.axis);
	...
	if (cent->currentState.weapon == WP_ROCKET_LAUNCHER && (cg.snap->uberRLUsers[cent->currentState.clientNum] == qtrue)) {
		memcpy(&orbs, &torso, sizeof(torso));
		orbs.hModel = cgs.media.uberRocketOrbModel;
		orbs.customSkin = 0;
		VectorCopy(torso.origin, orbs.origin);
		spinAngles[1] = cg.time * -0.2;
		AnglesToAxis(spinAngles, orbs.axis);
	...
	/* if (cent->currentState.weapon == WP_LIGHTNING && (cent->currentState.eFlags & EF_UW_LIGHTNING)) {
		memcpy(&orbs, &torso, sizeof(torso));
		orbs.hModel = cgs.media.uberLightningOrbModel;
		orbs.customSkin = 0;
		VectorCopy(torso.origin, orbs.origin);
		spinAngles[1] = cg.time * -0.2;
		AnglesToAxis(spinAngles, orbs.axis);
	...
	if (cent->currentState.weapon == WP_PLASMAGUN && (cent->currentState.eFlags & EF_UW_PLASMA)) {
		memcpy(&orbs, &torso, sizeof(torso));
		orbs.hModel = cgs.media.uberPlasmaOrbModel;
		orbs.customSkin = 0;
		VectorCopy(torso.origin, orbs.origin);
		spinAngles[1] = cg.time * -0.2;
		AnglesToAxis(spinAngles, orbs.axis);
	...
	if (cent->currentState.weapon == WP_RAILGUN && (cent->currentState.eFlags & EF_UW_RAIL)) {
		memcpy(&orbs, &torso, sizeof(torso));
		orbs.hModel = cgs.media.uberRailOrbModel;
		orbs.customSkin = 0;
		VectorCopy(torso.origin, orbs.origin);
		spinAngles[1] = cg.time * -0.2;
		AnglesToAxis(spinAngles, orbs.axis);
	...
	if (cent->currentState.weapon == WP_BFG && (cent->currentState.eFlags & EF_UW_BFG)) {
		memcpy(&orbs, &torso, sizeof(torso));
		orbs.hModel = cgs.media.uberBFGOrbModel;
		orbs.customSkin = 0;
		VectorCopy(torso.origin, orbs.origin);
		spinAngles[1] = cg.time * -0.2;
		AnglesToAxis(spinAngles, orbs.axis);
