...
void CG_RegisterWeapon( int weaponNum ) {
	...
	case WP_ROCKET_LAUNCHER:
		...
		weaponInfo->trackSound = trap_S_RegisterSound("sound/weapons/rocket/rocktrack.wav", qfalse);
...
static void CG_LightningBolt( centity_t *cent, vec3_t origin ) {
	...
	// with some help from PainKeep Arena's source code
...
//		VectorCopy(cg.refdef.vieworg, muzzlePoint );
	...
	/* VectorSubtract(trace.endpos, range, mins);
	VectorAdd(trace.endpos, range, maxs);

	trap_CM_BoxTrace(&ctrace, trace.endpos, &ctrace.endpos, mins, maxs, 0, MASK_SOLID); */
}
/*
...
========================
*/
/* static void CG_AddUberEffect(refEntity_t parent, centity_t *cent, weapon_t weaponNum, qboolean uber) {
	refEntity_t		orbs;
	vec3_t			spinAngles;

	if (weaponNum == WP_SHOTGUN && uber) {
		CG_Printf("ZZZZZZZZZZ");
		memcpy(&orbs, &parent, sizeof(parent));
		orbs.hModel = cgs.media.uberShotgunOrbModel;
		orbs.customSkin = 0;
		orbs.frame = 0;
		orbs.renderfx = RF_LIGHTING_ORIGIN;
		VectorCopy(parent.origin, orbs.origin);
...
void CG_DrawWeaponSelect( void ) {
	...
	if ( cg_weapons[ cg.weaponSelect ].item ) {
		...
		// EMERALD: Names for uberweapons
		if ((cg.predictedPlayerState.stats[STAT_UBERS] & UW_SHOTGUN) && (name == "Shotgun")) {
			name = "Explosive Shotgun";
		}
		if ((cg.predictedPlayerState.stats[STAT_UBERS] & UW_GRENADE) && (name == "Grenade Launcher")) {
			name = "Multi-Grenade Launcher";
		}
		if ((cg.predictedPlayerState.stats[STAT_UBERS] & UW_ROCKET) && (name == "Rocket Launcher")) {
			name = "Homing Rocket Launcher";
		}
		if ((cg.predictedPlayerState.stats[STAT_UBERS] & UW_LIGHTNING) && (name == "Lightning Gun")) {
			name = "Arc Lightning Gun";
		}
		if ((cg.predictedPlayerState.stats[STAT_UBERS] & UW_RAIL) && (name == "Railgun")) {
			name = "Toxic Railgun";
		}
		if ((cg.predictedPlayerState.stats[STAT_UBERS] & UW_PLASMA) && (name == "Plasma Gun")) {
			name = "Ion Plasma Gun";
		}
		if ((cg.predictedPlayerState.stats[STAT_UBERS] & UW_BFG) && (name == "BFG10K")) {
			name = "BFG30K";
		}
		if ( name ) {
