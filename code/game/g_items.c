...
int Pickup_Pouch(gentity_t *ent, gentity_t *other, qboolean captureMode) {
	...
	if (!captureMode) {
		other->client->weaponLimit += quantity;
		other->client->ps.stats[STAT_MAXWEAPONS] = other->client->weaponLimit;

		if (ent->item->classname == "item_pouch_ultra") {
			other->client->doublePickup = qtrue;
		}
	}
...
void Add_Ammo (gentity_t *ent, int weapon, int count)
{
	...
	if (weapon == WP_NONE)
	{
		...
		ent->client->ps.ammo[WP_MACHINEGUN] = amax(ent->client->ps.ammo[WP_MACHINEGUN], 150);
		ent->client->ps.ammo[WP_SHOTGUN] = amax(ent->client->ps.ammo[WP_SHOTGUN], 25);
		ent->client->ps.ammo[WP_GRENADE_LAUNCHER] = amax(ent->client->ps.ammo[WP_GRENADE_LAUNCHER], 25);
		ent->client->ps.ammo[WP_ROCKET_LAUNCHER] = amax(ent->client->ps.ammo[WP_ROCKET_LAUNCHER], 25);
		ent->client->ps.ammo[WP_LIGHTNING] = amax(ent->client->ps.ammo[WP_LIGHTNING], 150);
		ent->client->ps.ammo[WP_RAILGUN] = amax(ent->client->ps.ammo[WP_RAILGUN], 25);
		ent->client->ps.ammo[WP_PLASMAGUN] = amax(ent->client->ps.ammo[WP_PLASMAGUN], 150);
		ent->client->ps.ammo[WP_BFG] = amax(ent->client->ps.ammo[WP_BFG], 25);
		/* ent->client->ps.ammo[WP_MACHINEGUN] = 150;
		ent->client->ps.ammo[WP_SHOTGUN] = 25;
		ent->client->ps.ammo[WP_GRENADE_LAUNCHER] = 25;
		ent->client->ps.ammo[WP_ROCKET_LAUNCHER] = 25;
		ent->client->ps.ammo[WP_LIGHTNING] = 150;
		ent->client->ps.ammo[WP_RAILGUN] = 25;
		ent->client->ps.ammo[WP_PLASMAGUN] = 150;
...
int Pickup_Weapon (gentity_t *ent, gentity_t *other, qboolean CaptureMode) {
	...
	other->client->numWeapons++;
	other->client->ps.stats[STAT_WEAPONCOUNT] = other->client->numWeapons;

	if (other->client->doublePickup) {
		increase = 2;
		other->client->doublePickup = qfalse;
	}
	else {
		increase = 1;
	}

	// EMERALD: Code for uberweapons
	// Increment the weapon counter - once it reaches 3, upgrade to its uberweapon counterpart
	switch (ent->item->giTag) {
	case WP_SHOTGUN:
		if (other->client->shotgunCounter < 3) {
			other->client->shotgunCounter += increase;
			if (other->client->shotgunCounter == 3) {
				other->client->ps.stats[STAT_UBERS] |= UW_SHOTGUN;
				other->client->ps.eFlags |= EF_UW_SHOTGUN;
				other->flags |= EF_UW_SHOTGUN;
				PM_AddEvent(EV_EXPSHOTGUN_PICKUP);
			}
		}
		break;
	case WP_GRENADE_LAUNCHER:
		if (other->client->grenadeCounter < 3) {
			other->client->grenadeCounter += increase;
			if (other->client->grenadeCounter == 3) {
				other->client->ps.stats[STAT_UBERS] |= UW_GRENADE;
				other->client->ps.eFlags |= EF_UW_GRENADE;
			}
		}
		break;
	case WP_ROCKET_LAUNCHER:
		if (other->client->rocketCounter < 3) {
			...
			if (other->client->rocketCounter == 3) {
				other->client->ps.stats[STAT_UBERS] |= UW_ROCKET;
				other->client->ps.eFlags |= EF_UW_ROCKET;
			}
		}
		break;
	case WP_LIGHTNING:
		if (other->client->lightningCounter < 3) {
			other->client->lightningCounter += increase;
			if (other->client->lightningCounter == 3) {
				other->client->ps.stats[STAT_UBERS] |= UW_LIGHTNING;
				other->client->ps.eFlags |= EF_UW_LIGHTNING;
			}
		}
		break;
	case WP_PLASMAGUN:
		if (other->client->plasmaCounter < 3) {
			other->client->plasmaCounter += increase;
			if (other->client->plasmaCounter == 3) {
				other->client->ps.stats[STAT_UBERS] |= UW_PLASMA;
				other->client->ps.eFlags |= EF_UW_PLASMA;
			}
		}
		break;
	case WP_RAILGUN:
		if (other->client->railCounter < 3) {
			other->client->railCounter += increase;
			if (other->client->railCounter == 3) {
				other->client->ps.stats[STAT_UBERS] |= UW_RAIL;
				other->client->ps.eFlags |= EF_UW_RAIL;
			}
		}
		break;
	case WP_BFG:
		if (other->client->bfgCounter < 3) {
			other->client->bfgCounter += increase;
			if (other->client->bfgCounter == 3) {
				other->client->ps.stats[STAT_UBERS] |= UW_BFG;
				other->client->ps.eFlags |= EF_UW_BFG;
			}
		}
		break;
	default:
...
#else
	if (CaptureMode) {
		// other->client->capturedItem = ent->item;
...
void RespawnItem( gentity_t *ent ) {
	...
	if (ent->flags & FL_BOUNCED_ITEM) {
		// Com_Printf("Bounced item");
		ent->flags &= ~FL_DROPPED_ITEM;
		ent->s.pos.trType = TR_STATIONARY;
		G_SetOrigin(ent, ent->init_origin);
		// If the bounced item was originally on a mover, do some trace checks to ensure it gets back in
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	/* if (ent->item->giType == IT_WEAPON) {
		if (other->client->numWeapons >= (other->client->weaponLimit + 1))
			other->client->ps.eFlags |= EF_MAXWEAPS;
	} */
	...
	if (predict) {
		G_AddPredictableEvent( other, pickupEvent, ent->s.modelindex );
	} else {
...
// EMERALD: Bounced items use this as their think function to return to their original locations
void MoveBouncedItem(gentity_t *ent) {
	// G_SetOrigin(ent, ent->init_origin);
	RespawnItem(ent);
...
void G_SpawnItem (gentity_t *ent, gitem_t *item) {
	...
	// VectorCopy(ent->r.currentOrigin, ent->init_origin);
