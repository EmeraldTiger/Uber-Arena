	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			...
			if ((self->client->poisoner != NULL) && (self->client->ps.eFlags & EF_POISONED)) {
				// if victim is poisoned, both the fragger and the player who originally poisoned the victim get a frag
				// the poisoned victim also loses a frag
				AddScore( self, self->r.currentOrigin, -1);
				if (attacker != self->client->poisoner) {
					AddScore(self->client->poisoner, self->r.currentOrigin, 1);
					attacker->client->ps.persistant[PERS_PLAYEREVENTS] ^= PLAYEREVENT_POISONCREDIT;
				}
			}
			AddScore( attacker, self->r.currentOrigin, 1 );

			// BFG30K gives 3 frags instead of 1
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) {
		...
		if (ent->item && !(ent->s.eFlags & EF_NODRAW)) {
			...
			if (!(ent->flags & FL_BOUNCED_ITEM)) {
				ent->init_bouncetime = level.time;
			}
