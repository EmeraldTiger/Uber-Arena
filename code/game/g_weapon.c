...
void Bullet_Fire (gentity_t *ent, float spread, int damage ) {
	...
	for (i = 0; i < 10; i++) {
		...
		// EMERALD: Pick up items with bullets
		if (ent->client->ps.powerups[PW_SCAVENGER]) {
			trap_Trace(&tr, muzzle, NULL, NULL, end, passent, MASK_SHOT_SCAVENGER);
		}
		else {
...
qboolean ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
	...
	for (i = 0; i < 10; i++) {
		if (ent->client->ps.powerups[PW_SCAVENGER]) {
			trap_Trace(&tr, tr_start, NULL, NULL, tr_end, passent, MASK_SHOT_SCAVENGER);
		}
		else {
...
void Weapon_PendulumGun_Fire(gentity_t *ent) {
	gentity_t	*m;

	m = fire_pendulum(ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_railgun_fire (gentity_t *ent) {
	...
	passent = ent->s.number;
	do {
		trap_Trace (&trace, muzzle, NULL, NULL, end, passent, MASK_SHOT_SCAVENGER );
		...
		if ( traceEnt->takedamage ) {
			/* if (trace.surfaceFlags & SURF_LADDER) {
				G_BounceProjectile(muzzle, end, ent->s.pos.trDelta, end);
...
void Weapon_LightningFire( gentity_t *ent ) {
	...
	static vec3_t range = { 512, 512, 512 };
	vec3_t		hitdir;
	trace_t		los;
	int j;
	...
	for (i = 0; i < 10; i++) {
		...
		if (ent->client->lightningCounter >= 3) {
			...
			for (j = 0; j < num; j++) {
				...
				/* if (hit->client->ps.pm_type == PM_DEAD)
					continue; */
...
convenience function that returns the weapon's corresponding counter (for uberweapon-related code)
===============
*/
int *GetWeaponCounter(int weapon, gentity_t *other) {
	int	*weaponPtr;
	switch (weapon) {
	case WP_SHOTGUN:
		weaponPtr = &other->client->shotgunCounter;
		break;
	case WP_GRENADE_LAUNCHER:
		weaponPtr = &other->client->grenadeCounter;
		break;
	case WP_ROCKET_LAUNCHER:
		weaponPtr = &other->client->rocketCounter;
		break;
	case WP_LIGHTNING:
		weaponPtr = &other->client->lightningCounter;
		break;
	case WP_PLASMAGUN:
		weaponPtr = &other->client->plasmaCounter;
		break;
	case WP_RAILGUN:
		weaponPtr = &other->client->railCounter;
		break;
	default:
		return NULL;
		break;
...
============== =
*/
void SetWeaponCounter(int weapon, gentity_t *other, int value) {
	switch (weapon) {
	case WP_SHOTGUN:
		other->client->shotgunCounter = value;
		break;
	case WP_GRENADE_LAUNCHER:
		other->client->grenadeCounter = value;
		break;
	case WP_ROCKET_LAUNCHER:
		other->client->rocketCounter = value;
		break;
	case WP_LIGHTNING:
		other->client->lightningCounter = value;
		break;
	case WP_PLASMAGUN:
		other->client->plasmaCounter = value;
		break;
	case WP_RAILGUN:
		other->client->railCounter = value;
		break;
	default:
		break;
	}
