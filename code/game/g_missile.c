...
void G_BounceMissile( gentity_t *ent, trace_t *trace, int original, qboolean done ) {
	...
	// Check for client too, because otherwise grenade shooter entities will crash the game
	if (ent->parent->client && ent->parent->client->grenadeCounter >= 3) {
		...
		// values get modified in the grenade fire function.
		holdEnt = *ent;
...
void G_RunMissile(gentity_t *ent) {
	...
	int			targets[MAX_GENTITIES];
	vec3_t		mins, maxs;
	static vec3_t	range = { 2048, 2048, 2048 };
	int			num;
	gentity_t	*spotted;
	vec3_t		dirnew, dir;
	int			rocketlength;
	...
	vec3_t		finaldir;
	float		speed;
	vec3_t		dirnewnorm, dirnorm, deltanorm;
	trace_t		vtr;
	trace_t		ptr;
	vec3_t		endview;
	vec3_t		error = { 20, 20, 20 };
	vec3_t		targetOrigin, rocketOrigin;
	float		dirchange;
	vec3_t		forward;
	float		finallength;
	int			i;
	vec3_t		pforward, pright, pup;
	trace_t		pendtr;
	int			k;
	gentity_t	*pendEnt;
	vec3_t		pendDir;
	...
	VectorMA(ent->r.currentOrigin, 8192 * 16, forward, aim);

	trap_Trace(&vtr, ent->r.currentOrigin, mins, maxs, aim, passent, ent->clipmask); */

	/* if (vtr.contents & CONTENTS_BODY)
		Com_Printf("Locked-On"); */

		// EMERALD
		// trap_Trace(&pgtr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, MASK_SHOT);

		/* if (pgtr.startsolid)
			Com_Printf("bounce"); */

	if (ent->classname == "pendulum") {
		AngleVectors(ent->parent->client->ps.viewangles, pforward, pright, pup);
		// VectorNormalize(ent->s.pos.trDelta);
		VectorCopy(pforward, ent->s.pos.trDelta);
		VectorScale(pright, 5, pright);
		Com_Printf("%f \n", pforward[1]);
		VectorScale(ent->s.pos.trDelta, 400, ent->s.pos.trDelta);
		VectorCopy(ent->parent->r.currentOrigin, ent->s.pos.trBase);
		trap_Trace(&pendtr, ent->s.pos.trBase, NULL, NULL, ent->r.currentOrigin, ent->r.ownerNum, MASK_SHOT);
		for (k = 0; k < MAX_GENTITIES; k++) {
			pendEnt = &g_entities[pendtr.entityNum];
		}
		if (pendEnt->takedamage) {
			// Com_Printf("Caught something!");
			// G_Damage(pendEnt, ent, ent->parent, ent->s.pos.trDelta, pendEnt->r.currentOrigin, 15, 0, MOD_GRAPPLE);
			// VectorScale(pright, 5, pright);
			// Com_Printf("%i \n", VectorLength(pright));
			G_RadiusDamage(pendEnt->r.currentOrigin, ent->parent, (8), 2, ent->parent, MOD_ROCKET);
			VectorAdd(pendEnt->s.pos.trDelta, ent->s.pos.trDelta, pendEnt->s.pos.trDelta);
			VectorSubtract(pendEnt->r.currentOrigin, ent->r.currentOrigin, pendDir);
			// VectorScale(pendDir, 2, pendDir);
			// VectorCopy(pendDir, pendEnt->client->ps.velocity);
			// VectorNormalize(pendEnt->s.pos.trDelta);
		}
		// VectorScale(ent->s.pos.trDelta, 800, ent->s.pos.trDelta);
		// VectorScale(ent->s.pos.trDelta, 0.9, ent->s.pos.trDelta);
	}
	...
	if (ent->parent->client->rocketCounter >= 3 && ent->classname == "rocket") {
		...
		for (i = 0; i < level.maxclients; i++) {
			...
			// need line-of-sight to victim to track properly
		...
		if (candidate) {
			...
			//VectorScale(ent->s.pos.trDelta, 1.5, ent->s.pos.trDelta);
			// min(VectorLength(ent->s.pos.trDelta), 80);
		}
		// Com_Printf("%i \n", shortestDist);
			// ent->locked = qtrue;
			// vtos(dir);
			/* VectorCopy(ent->s.pos.trDelta, entangles);
			VectorNormalize(entangles);
			/* VectorAdd(finaldir, entangles, finaldir);
			VectorNormalize(finaldir);
			vectoangles(finaldir, ent->r.currentAngles);
			speed = VectorLength(ent->s.pos.trDelta);
			min(80, speed);
			Com_Printf("%f \n", speed);
			VectorScale(finaldir, speed, finaldir);
			VectorCopy(finaldir, ent->s.pos.trDelta);
			// ent-> */
	}
	...
	if (ent->parent->client) {
		if (ent->parent->client->plasmaCounter >= 3 && ent->classname == "plasma") {
			...
			// G_AddEvent(ent, EV_MISSILE_HIT, DirToByte(tr.plane.normal));
			// ent->s.otherEntityNum = traceEnt->s.number;
			// G_Damage(traceEnt, ent, ent->parent, NULL, NULL, 20, DAMAGE_RADIUS, MOD_PLASMA);
		}
	}
...
=================
*/
gentity_t *fire_pendulum(gentity_t *self, vec3_t start, vec3_t dir) {
	gentity_t	*bolt;

	VectorNormalize(dir);

	bolt = G_Spawn();
	bolt->classname = "pendulum";
	bolt->nextthink = level.time + 2000;
	bolt->think = G_ExplodeMissile;
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_USE_CURRENT_ORIGIN;
	bolt->s.weapon = WP_PENDULUMGUN;
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	bolt->damage = 25;
	bolt->splashDamage = 0;
	...
	bolt->s.pos.trType = TR_SINE;
	bolt->s.pos.trDuration = 4000;
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
	VectorCopy(start, bolt->s.pos.trBase);
	VectorScale(dir, 900, bolt->s.pos.trDelta);
	SnapVector(bolt->s.pos.trDelta);			// save net bandwidth
