...
=================
*/
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd, qboolean maxweaps ) {
	pmove_t	pm;
	gclient_t	*client;
	...
	if ( client->sess.spectatorState != SPECTATOR_FOLLOW ) {
		...
		// perform a pmove
		Pmove (&pm, maxweaps, qfalse, qfalse);
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual >= 1000 ) {
		...
		/* if (ent->client->ps.eFlags & EF_POISONED && ent->client->ps.stats[STAT_HEALTH] < ent->client->baseHealth) {
			ent->health += 10;
		} */
	...
	/*if (ent->client->ps.eFlags & EF_UW_ROCKET) {
		PM_AddEvent(EV_HOMINGROCKET_PICKUP);
	}
	else {
		PM_AddEvent(EV_HOMINGROCKET_DROP);
	} */
#ifdef MISSIONPACK
...
//	qboolean	fired;
	...
	for ( i = oldEventSequence ; i < client->ps.eventSequence ; i++ ) {
		...
		case EV_EXPSHOTGUN_PICKUP:
			ent->client->ps.eFlags |= EF_UW_SHOTGUN;
			// Com_Printf("will this print also?");
			break;

		case EV_EXPSHOTGUN_DROP:
			ent->client->ps.eFlags &= ~EF_UW_SHOTGUN;
			// Com_Printf("will this print?");
			break;

		case EV_MULTIGRENADE_PICKUP:
			ent->client->ps.eFlags |= EF_UW_GRENADE;
			break;

		case EV_MULTIGRENADE_DROP:
			ent->client->ps.eFlags &= ~EF_UW_GRENADE;
			break;

		case EV_HOMINGROCKET_PICKUP:
			ent->client->ps.eFlags |= EF_UW_ROCKET;
			break;

		case EV_HOMINGROCKET_DROP:
			ent->client->ps.eFlags &= ~EF_UW_ROCKET;
			break;
		...
		case EV_USE_ITEM7:		// tuning device
			switch (pm->ps->weapon) {
			case WP_SHOTGUN:
				if (ent->client->shotgunCounter < 3) {
					ent->client->shotgunCounter = 3;
					ent->client->ps.stats[STAT_UBERS] |= UW_SHOTGUN;
				}
				else
					Com_Printf("Weapon is already an uberweapon.\n");
				break;
			case WP_GRENADE_LAUNCHER:
				if (ent->client->grenadeCounter < 3) {
					ent->client->grenadeCounter = 3;
					ent->client->ps.stats[STAT_UBERS] |= UW_GRENADE;
				}
				else
					Com_Printf("Weapon is already an uberweapon.\n");
				break;
			case WP_ROCKET_LAUNCHER:
				if (ent->client->rocketCounter < 3) {
					ent->client->rocketCounter = 3;
					ent->client->ps.stats[STAT_UBERS] |= UW_ROCKET;
				}
				else
					Com_Printf("Weapon is already an uberweapon.\n");
				break;
			case WP_LIGHTNING:
				if (ent->client->lightningCounter < 3) {
					...
					ent->client->ps.stats[STAT_UBERS] |= UW_LIGHTNING;
				}
				else
					Com_Printf("Weapon is already an uberweapon.\n");
				break;
			case WP_PLASMAGUN:
				if (ent->client->plasmaCounter < 3) {
					ent->client->plasmaCounter = 3;
					ent->client->ps.stats[STAT_UBERS] |= UW_PLASMA;
				}
				else
					Com_Printf("Weapon is already an uberweapon.\n");
				break;
			case WP_RAILGUN:
				if (ent->client->railCounter < 3) {
					ent->client->railCounter = 3;
					ent->client->ps.stats[STAT_UBERS] |= UW_RAIL;
				}
				else
					Com_Printf("Weapon is already an uberweapon.\n");
				break;
			case WP_BFG:
				if (ent->client->bfgCounter < 3) {
					...
					ent->client->ps.stats[STAT_UBERS] |= UW_BFG;
				}
				else
					Com_Printf("Weapon is already an uberweapon.\n");
				break;
			default:
				Com_Printf("Weapon cannot be upgraded.\n");
				break;
			}
			break;
...
//		G_Printf("serverTime >>>>>\n" );
	...
	if (ent->client->numWeapons >= (ent->client->weaponLimit + 1)) {
		maxweaps = qtrue;
	}
	else {
		maxweaps = qfalse;
	}
	// ent->client->ps.eFlags |= EF_MAXWEAPS;
	...
	if ( client->sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		SpectatorThink( ent, ucmd, maxweaps );
		return;
	}
	...
	/* if (g_UpsideDown.integer)
		pm.ps->eFlags |= EF_UPSIDEDOWN;
	else
		pm.ps->eFlags &= ~EF_UPSIDEDOWN; */
	...
	// upsideDown = qfalse;
		...
		Pmove (&pm);
#else
	if (ent->client->receptacleMode == REC_STANDBY) {
		finished = qtrue;
	}
		// EMERALD
		// Pmove can't access game-specific functionality, so pass in variables through this function when needed
		Pmove (&pm, maxweaps, &upsideDown, &finished);
#endif
	...
	// EMERALD: Toxic railgun status effects
	// Health regen occurs in Client Timer Actions
	...
	/* if (ent->client->ps.eFlags & EF_UW_GRENADE) {
		PM_AddEvent(EV_MULTIGRENADE_PICKUP);
	}
	else {
		PM_AddEvent(EV_MULTIGRENADE_DROP);
	} */
	...
	if (ent->client->ps.eFlags & EF_UW_LIGHTNING) {
		PM_AddEvent(EV_ARCLIGHTNING_PICKUP);
	}

	if (ent->client->ps.eFlags & EF_UW_PLASMA) {
		PM_AddEvent(EV_IONPLASMA_PICKUP);
	}

	if (ent->client->ps.eFlags & EF_UW_RAIL) {
		PM_AddEvent(EV_TOXICRAIL_PICKUP);
	}

	if (ent->client->ps.eFlags & EF_UW_BFG) {
		PM_AddEvent(EV_BFG30K_PICKUP);
	} */
